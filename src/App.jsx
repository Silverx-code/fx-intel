...
function generatePriceHistory(base, points = 60) {
  ...
  for (let i = points; i >= 0; i--) {
    ...
    data.push({ time: ts.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }), price: parseFloat(price.toFixed(4)), vol: Math.floor(Math.random() * 800 + 200) });
  }
  return data;
}
...
function ConfirmModal({ order, onConfirm, onCancel, loading }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999, backdropFilter: "blur(4px)" }}>
      <div style={{ background: "#0d1420", border: `1px solid ${isBuy ? "rgba(0,255,136,0.3)" : "rgba(255,68,102,0.3)"}`, borderRadius: 20, padding: 32, width: 380, animation: "fadeSlideIn 0.2s ease" }}>
...
function ChatMessage({ msg }) {
  ...
  function renderText(text) {
    return text.split("\n").map((line, i) => {
      if (line.trim().startsWith("- ") || line.trim().startsWith("• ")) return <div key={i} style={{ display: "flex", gap: 8, marginTop: 4 }}><span style={{ color: "#00ff88", flexShrink: 0 }}>▸</span><span>{renderInline(line.trim().slice(2))}</span></div>;
  ...
  return (
    <div style={{ display: "flex", gap: 12, flexDirection: isUser ? "row-reverse" : "row", alignItems: "flex-start", marginBottom: 20, animation: "fadeSlideIn 0.3s ease" }}>
...
export default function FXIntelligence() {
  ...
  const [chatMessages, setChatMessages] = useState([{
    role: "assistant",
    content: "Hello! I'm your FX Intelligence AI analyst. I have live access to all your prices, signals, portfolio, and news.\n\nOnce you connect your Bybit keys in the **Trade** tab, I can also help you plan entries and exits with real market data.\n\n**What would you like to analyse?**",
  ...
  const fetchOrderHistory = async () => {
    try {
      const res = await fetch(`${PROXY}/orders/history`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ apiKey, apiSecret }) });
      const data = await res.json();
  ...
  const buildContext = useCallback(() => {
    ...
    const bybitLines = Object.entries(liveTickerData).map(([sym, t]) => `${sym}: $${t.price} (24h: ${(parseFloat(t.change24h||0)*100).toFixed(2)}%)`).join("\n") || "Not connected";
    const balLines = balances.map(b => `${b.coin}: ${b.available} available ($${parseFloat(b.usdValue||0).toFixed(2)})`).join("\n") || "Not connected";
    return `You are FX·INTEL, an expert FX and crypto trading analyst. Be direct and data-driven. Use **bold**, - bullet points, ### headers. Always give a clear bottom line.
...
Give grounded, actionable analysis. Brief disclaimer if suggesting trades.`;
  }, [priceData, signals, liveTickerData, balances]);

  const sendMessage = useCallback(async (text) => {
    const msg = (text || chatInput).trim();
    if (!msg || chatLoading) return;
    setChatMessages(prev => [...prev, { role: "user", content: msg, timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }, { role: "assistant", content: "", loading: true }]);
    setChatInput("");
    setChatLoading(true);
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, system: buildContext(), messages: [...history, { role: "user", content: msg }] }),
      });
      const data = await res.json();
      setChatMessages(prev => [...prev.slice(0,-1), { role: "assistant", content: data.content?.[0]?.text || "Error getting response.", timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }]);
    } catch {
  ...
  const s = {
    ...
    cardGreen: { background: "rgba(0,255,136,0.06)", border: "1px solid rgba(0,255,136,0.2)", borderRadius: 16, padding: 20 },
    ...
    <div style={s.app}>
      ...
      <div style={s.header}>
        <div style={s.logo}>
          ...
          {proxyOnline
            ? <div style={{ fontSize: 10, color: "#00ff88", background: "rgba(0,255,136,0.1)", border: "1px solid rgba(0,255,136,0.25)", padding: "2px 8px", borderRadius: 20 }}>⬡ Bybit Connected</div>
      ...
      <div style={s.main}>
        ...
        {tab === "trade" && (
          <div>
            <div style={{ fontSize: 20, fontFamily: "'Syne',sans-serif", marginBottom: 20, color: "#fff" }}>⬡ Trade on Bybit</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 20 }}>
              ...
              <div style={{ ...s.card, gridColumn: credsSaved ? "auto" : "1 / span 2" }}>
                <div style={s.label}>🔑 Bybit API Credentials</div>
                {credsSaved ? (
                  <div>
                    <div style={{ fontSize: 13, color: "#00ff88", marginBottom: 12 }}>✓ Connected</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {balances.length > 0 ? balances.map(b => (
                        <div key={b.coin} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "8px 12px", background: "rgba(255,255,255,0.03)", borderRadius: 8 }}>
                          ...
                          <span style={{ color: "rgba(255,255,255,0.4)" }}>${parseFloat(b.usdValue||0).toFixed(2)}</span>
                        </div>
                      )) : <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)" }}>No balances found (empty wallet?)</div>}
                    </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      <div>
                        ...
                        <input style={s.input} type="text" placeholder="Paste your Bybit API Key" value={apiKey} onChange={e => setApiKey(e.target.value)} />
                      </div>
                      <div>
                        <div style={{ ...s.label, marginBottom: 6 }}>API Secret</div>
                        <input style={s.input} type="password" placeholder="Paste your Bybit API Secret" value={apiSecret} onChange={e => setApiSecret(e.target.value)} />
                      </div>
                      {credsError && <div style={{ fontSize: 12, color: "#ff4466", background: "rgba(255,68,102,0.08)", padding: "8px 12px", borderRadius: 8 }}>{credsError}</div>}
                      <button style={{ ...s.btn, width: "100%" }} onClick={connectBybit} disabled={balanceLoading}>
                        {balanceLoading ? "Connecting..." : "Connect Bybit Account"}
                      </button>
                    </div>
                  </div>
                )}
                  ...
                  <div style={{ ...s.card, flex: 1, overflow: "hidden" }}>
                    ...
                    <div style={{ maxHeight: 250, overflowY: "auto" }}>
                      {orderHistory.length === 0
                        ? <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", textAlign: "center", padding: "16px 0" }}>No order history</div>
                        : orderHistory.slice(0, 10).map(o => (
                          <div key={o.orderId} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 11, padding: "8px 10px", background: "rgba(255,255,255,0.02)", borderRadius: 6, marginBottom: 4 }}>
                            ...
                            <span style={{ fontSize: 10, color: o.orderStatus === "Filled" ? "#00ff88" : o.orderStatus === "Cancelled" ? "#ff4466" : "#ffaa00" }}>{o.orderStatus}</span>
                          </div>
                        ))
                      }
                    </div>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                        <div>
                          <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
                            ...
                            <div style={{ color: chg >= 0 ? "#00ff88" : "#ff4466", fontSize: 14 }}>
                              {chg >= 0 ? "▲" : "▼"} {Math.abs(chg).toFixed(4)}
                            </div>
                          </div>
                        </div>
        ...
        {tab === "signals" && (
          <div>
            <div style={{ fontSize: 20, fontFamily: "'Syne',sans-serif", marginBottom: 20, color: "#fff" }}>◈ All ML Signals</div>
              ...
              {CURRENCIES.map(pair => {
                ...
                return (
                  <div key={pair} style={{ ...s.card, border: `1px solid ${sig2.color}33` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                      <div><div style={{ fontSize: 16, fontWeight: 700 }}>{CURRENCY_ICONS[pair]} {pair}</div><div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>{price.toFixed(4)}</div></div>
        ...
        {tab === "sentiment" && (
          <div>
            <div style={{ fontSize: 20, fontFamily: "'Syne',sans-serif", marginBottom: 20, color: "#fff" }}>◉ News & Market Sentiment</div>
            <div style={s.card}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {ECONOMIC_EVENTS.map((ev, i) => (
                <div key={i} style={{ ...s.card, display: "flex", alignItems: "center", gap: 16 }}>
                  <div style={{ width: 60, textAlign: "center" }}><div style={{ fontSize: 14, fontWeight: 700 }}>{ev.time}</div><div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)" }}>Today</div></div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 16, height: "calc(100vh - 180px)", minHeight: 600 }}>
            <div style={{ ...s.card, display: "flex", flexDirection: "column", padding: 0, overflow: "hidden" }}>
              <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", gap: 12, background: "rgba(0,255,136,0.04)" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 10, color: "#00ff88", marginTop: 2 }}>● Live data · {CURRENCIES.length} FX pairs · {TRADE_SYMBOLS.length} crypto pairs{credsSaved ? " · Bybit connected" : ""}</div>
                </div>
              ...
              <div style={{ padding: "12px 16px", borderTop: "1px solid rgba(255,255,255,0.06)", background: "rgba(0,0,0,0.2)" }}>
                <div style={{ display: "flex", gap: 10, alignItems: "flex-end" }}>
                  <textarea ref={inputRef} value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} placeholder="Ask about any pair, signals, portfolio, macro risks..." rows={2} style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 10, padding: "10px 14px", color: "#e2e8f0", fontSize: 12, fontFamily: "inherit", resize: "none", outline: "none", lineHeight: 1.5 }} />
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {credsSaved && balances.length > 0 && (
                <div style={s.card}>
                  <div style={s.label}>Bybit Balance</div>
                  {balances.map(b => (
